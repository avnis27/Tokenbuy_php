...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * caching of user and their groups
	 *
	 * @var array
	 **/
	 ...
	 * against an entry in the user table.
	 ...
	 * Deactivae : Updates a user row with an activation code.
	 *
	 * @author Mathew
	 */
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['user'], $data, array('user_id' => $id));
		}
		else
		{
			$data = array(
	...
	public function backup_codes($id)
	{
		...
		for($n=0;$n<$this->otp['backup_codes_length'];$n++)
		{
			...
			// Only take a specific length from the hash_code function for easier login for user
		...
		$this->db->update($this->tables['user'], $update);
	...
	public function delete_backup_code($id, $current_backup_codes = array(), $backup_code){
		...
		if($this->db->update($this->tables['user'], $data))
		{
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('forgotten_password_code', $code)->user()->row(); //pass the code to profile
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    'created_on' => time(),
		    'last_login' => time(),
		    'isActive'     => ($manual_activation === false ? 1 : 0)
		);
		...
		//filter out any data passed that doesnt have a matching column in the user table
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', user_email, user_email, user_id, user_password, otp, isActive, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
	    			  ->order_by('user_id', 'desc')
		                  ->get($this->tables['user']);
	...
	public function otp_login($identity, $token, $remember=FALSE, $secret_key){
		...
		$query = $this->db->select($this->identity_column . ', user_email, user_email, user_id, otp, otp_login_code, last_login')
		...
		if ($query->num_rows() === 1)
		{
			...
			if($this->is_otp_secret_code_valid($user->otp_login_code, $secret_key))
			{
				...
				else if($this->is_backup_code_valid($user->user_id, $token) && $this->otp['backup_codes_enabled'])
				{
					$this->set_session($user);
	...
	*/
	public function set_otp_secret_key($id)
	{
		...
		$query = $this->db->update($this->tables['user'], $update, array('user_id' => $id));
		if ($query) {
	...
	*/
	...
	public function is_otp_set($identity)
	{
		...
		else
		{
			...
			//$result = $this->db->select('otp')->get_where('user', array('user_email'=>$this->escape_str($identity)))->row();
			if($result->otp != '')
			{
	...
	*/
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['user'].'.*',
			    $this->tables['user'].'.user_id as id',
			    $this->tables['user'].'.user_id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['user'].'='.$this->tables['user'].'.user_id',
				    'inner'
				);
			}
	...
	public function user($id = NULL)
	{
		...
		//if no id was passed use the current user id
		$id || $id = $this->session->userdata('user_id');

		$this->limit(1);
		$this->order_by('user_id', 'desc');
		$this->where($this->tables['user'].'.user_id', $id);
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['user'] => (int)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id, $this->join['user'] => (int)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['users_groups'], array($this->join['user'] => (int)$user_id))) {
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['user'], $data, array('user_id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		// delete user from user table should be placed after remove from group
		$this->db->delete($this->tables['user'], array('user_id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['user'], array('last_login' => time()), array('user_id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['user'], array('remember_code' => $salt), array('user_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', user_id, user_email, user_email, last_login')
		...
		if ($query->num_rows() == 1)
		{
			...
			//extend the user cookies if the option is enabled
